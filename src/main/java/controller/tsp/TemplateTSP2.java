...
public abstract class TemplateTSP2 implements TSP2 {
	...
	private void completeRound(Date dateStart){
		Date arrivalDate = new Date(dateStart.getTime() + coutMeilleureSolution);
		for(DeliveryTime[] roundTab : roundList){
			roundTab[roundTab.length -1] = new DeliveryTime(roundList.get(0)[0].getCheckpoint(), arrivalDate,null,0);
		}
	}
	...
	public int chercheSolution(int tpsLimite, int nbSommets, int[][] cout, int[] duree, List<Checkpoint> checkpointList){
		...
		branchAndBound(new DeliveryTime(checkpointList.get(0), null,null, 0), nonVus, vus, 0, cout, duree, checkpointList.get(0).getTimeRangeStart().getTime(), tpsLimite);
	 ...
	 */	
	 void branchAndBound(DeliveryTime sommetCrt, List<DeliveryTime> nonVus, ArrayList<DeliveryTime> vus, long coutVus, int[][] cout, int[] duree, long tpsDebut, int tpsLimite){
		...
		if(waitingTime != -1){
		    ...
		    if (nonVus.size() == 0){ // tous les sommets ont ete visites
		    ...
		    } else if (coutVus + waitingTime + bound(sommetCrt.getCheckpoint(), nonVus, cout, duree) < 
		        ...
		        while (it.hasNext()){
		        	...
		        	branchAndBound(prochainSommet, nonVus, vus, coutVus + waitingTime + cout[indexValues.get(sommetCrt.getCheckpoint().getId())][indexValues.get(prochainSommet.getCheckpoint().getId())] + duree[indexValues.get(prochainSommet.getCheckpoint().getId())], cout, duree,tpsDebut, tpsLimite);
